package com.ezenbook.util.interceptor;
...
import com.ezenbook.member.vo.LoginVO;
...
public class AuthrityInterceptor extends HandlerInterceptorAdapter {
   ...
   static {
      // 게시판 - 등록, 수정, 삭제 - 관리자 :9
      authMap.put("/books/write.do", 9);
      authMap.put("/books/update.do", 9);
      authMap.put("/books/delete.do", 9);
   ...
   public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
      ...
      if (!checkAuthority(vo)) {
         // 오류 페이지로 이동시킵니다.
         response.sendRedirect(request.getContextPath() + "/error_page.do");
